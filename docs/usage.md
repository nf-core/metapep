...
**Assembly selection and fallback (auto-selected assemblies):** When no assembly_id is given for a taxon, the pipeline uses Entrez elink to retrieve all assemblies linked to that taxon, ranks them by total genome length (largest first), and initially selects the largest. If the selected assembly has no nucleotide sequence links in Entrez (missing or empty LinkSetDb/Link entries), the pipeline automatically falls back to the next ranked candidate and retries. This continues until a valid assembly is found or all candidates are exhausted. If no valid assembly is found for a taxon, it is excluded from downstream analysis. User-specified assembly_ids are used as-is and have no automatic fallback. Finally, the selected assembly per taxon is recorded in `logs/download_proteins.log` and its protein annotation is retrieved.
