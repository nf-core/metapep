...
- [SYFPEITHI](http://www.syfpeithi.de/index.html): The score represents how well a peptide is able to bind in comparison to all annotated binders of a specific allele. Therefore, the final scores are normalized to the highest scoring ligand for each allele, resulting in scores between 0 and 1. For downstream processes such as `prepare_entity_binding_ratios` peptides with a score of ≥0.5 are classified as binders.
