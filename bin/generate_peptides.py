        ...
        for k in range(args.min_len, args.max_len + 1):
            ...
            results = pd.DataFrame(
                [
                    ...
                    for pep in gen_peptides(it.protein_sequence, k)
                ],
            ...
            unique_peptides[["peptide_id", "peptide_sequence"]].to_csv(
                pep_handle, mode="a", sep="\t", index=False, header=False
            )

            results = results.merge(unique_peptides)
            results[["protein_id", "peptide_id", "count"]].to_csv(
                args.proteins_peptides, mode="a", sep="\t", index=False, header=False
            )
