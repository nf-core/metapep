...
def parse_args():
    ...
    parser.add_argument("-cond",    "--conditions-microbiomes",         required=True, metavar="PATH", type=str,                              help="Conditions - microbiomes map (condition_id, microbiome_id)")
...
# Read and join the table that connects the condition_id to the microbiome_id
conditions_microbiomes = pd.read_csv(args.conditions_microbiomes, sep='\t')
entity_condition = entity_microbiome.merge(conditions_microbiomes)
